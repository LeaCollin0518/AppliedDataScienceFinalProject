...
We see that December, June and January have the highest average sale price (going higher than 1 million dollars) whereas properties sold in March, September remain less expensive. We also see that the number of properties sold remains fairly consistent across all months.
...
Overall, the number of sales in New York City dropped during the first 7 years of the observed time period and then slightly rose between 2009 and 2010. The likely reason for the huge drop is the subprime mortgage crisis that we have already observed in a few of our graphs. We see the number of sales drop from 125,000 in 2006 to below 75,000 in 2010. After 2010, the housing market starts to recover and rises to about 87,000 from 2010 to 2012. After 2012, it remains at this level, still far from the numbers in 2003. 

As for the trend in each borough, Staten Island and the Bronx always had the lowest sales numbers and Queens almost always had the highest. All 5 boroughs were affected by the housing crisis. Apart from Manhattan, all other boroughs had similar trends to the overall trend. Manhattan sales numbers fluctuated more during this time frame. Numbers were rising slightly from 2003 to 2006 whereas all other borough sales were decreasing. It was decreasing slightly from 2013 to 2017 while the other boroughs were increasing their numbers. The explanation for this requires further investigation, perhaps into data with population, economics and financial conditions in each borough. 

Now we look more into the building class codes. Here, we visualize the number of buildings of each code class in the dataset in each borough. Because there are so many building codes and many had such a negligible amount sold in the data, we plotted only the rows that had more than 250 sold in the 15 years. Note: we are showing the raw counts, not the proportions. 
...
We can see if there is any relationship now between tax class at sale price. 
...
Now we'll take a look at how the total number of units and the square footage affect price. Without loss of generality, we will mainly focus on total units and land square feet here (and in fact the `total units` is just the sum of the residential units and commercial).
...
We are **finally** ready to fit some machine learning models. 
...
As finance center in the world, the real estate market in New York is also closely related to the finance market. The results shows that, overall speaking, New York estate market suffers from the subprime mortgage crisis which happened from 2006 to 2010. There was a huge drop both in house price and in sales number, and till now, the market have not recovered to the level of 2003 yet. 

As for the condition in each borough, apparently, Manhattan is the most expensive borough, Brooklyn is the second. House prices in Queens and Staten Island are on the similar level and these 2 boroughs are the cheapest in New York. And in general, area that close to Manhattan tend to have higher price. 
From the regression model, we can see that parameter 'neighborhood' is a significant parameter, which confirms the point of "Location! Location! Location!". And the newer the building is, the higher the house price can be. As for the built year, it depends on different age, there is no trend can be found. 

# Assumptions

Firstly, we made an assumption that our original data has contained the main factors that might affect New York real estate prices. In fact, from our daily experience, those most important factors when we considering the house price like geographical location, purchase time, housing type have all been concluded in our data. In our models, the significance of these factors and their impact on prices can also partly prove the rationality of this assumption.

# Limitations and Uncertainties

First, apart from missing values, some of the record is obviously not correct. Such that in some of the record, the house price is 1 or some small numbers that cannot be the house price. What we do is that, when facing problems includes house price, only records with house price larger than 500000 are used, which is a rough selection. Records with  correct numbers maybe moved out and records with wrong number can still exists in the data we are using, which may cause baise in our analysis. 

Second, the dataset we are using does not contain every house sale records. The data can be collected intensively from some specific neighborhood, and for some neighborhood, data can be relatively missing. This can lead to results with bias, such as the mean price per foot in Brooklyn is higher than that of Manhattan. A more accurate analysis can be done by a resample with equal sample size from each neighborhood. 

# Areas of Future Investigation

All the analysis was done based on the real estate data in New York city, however, as the finance center in the world, house market in New York must be closely related to the finance market and economics condition. Some economics factor such as GDP, CPI, population density should be added in the data in the future investigation.

Locations is not just about neighborhood and borough, it also includes distance to and number of underground station, restaurants, whole food stores and so on. A high quality neighborhood with high living condition obviously can be more expensive than the neighborhood without any living facilities. However, due to the data limitation, investigation from living condition aspect cannot be done. It can be finished with a more complete dataset. 

All our analysis is based on the data from the 5 boroughs in New York city, however, through Jersey City and Long Island City are not  New York administratively, they can be include to the Great New York area geographically, as we all know, a great amount people actually live in this two area and work in New York. There must be more interesting result can be found if the data from this 2 area can be include in the future investigation. 
