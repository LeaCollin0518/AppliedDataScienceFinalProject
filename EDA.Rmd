...
```{r}
unique(all.data[,get(tax.class.name)])
unique(all.data[,get(tax.class.sale.name)])

text.borough <- dat[, .N, by = c(tax.class.name, borough.name)]
text.sale.borough <- dat[, .N, by = c(tax.class.sale.name, borough.name)]

ggplot(data = text.borough, mapping = aes(x = get(tax.class.name), y = N)) +
  geom_bar(stat = "identity", color = "blue", fill = "#99CCFF") +
  labs(x = "Group of Total Units", y = "Number of Cases in each Group") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
ggplot(data = dat, aes(x = get(tax.class.name), y = get(log.price.name))) +
  geom_boxplot() +
  labs(title = "Logarithm of NYC Real Estate Price by Total Units", x = "Tax class at present", y = "Log Sale Price") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(data = dat, aes(x = as.factor(get(tax.class.sale.name)), y = get(log.price.name))) +
  geom_boxplot() +
  labs(title = "Logarithm of NYC Real Estate Price by Total Units", x = "Tax class at time of sale", y = "Log Sale Price") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
