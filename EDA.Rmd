...
As we can see, the range of values for sale price is enormous. Some buildings are up to 2 billion dollars! This is because the dataset includes any sale of property. Some properties are one-family homes or walk-up apartments while others are office buildings or retail buildings. The most expensive buildings that are likely office spaces can be found in Downtown or Midtown Manhattan. This makes sense as this is where the corporate side of New York resides.  
...
#### plot of number of house sales in each borough
```{r}
unique(all.data$BOROUGH)
a <- matrix(0, nrow = 5, ncol = 2)
colnames(a) <- c('borough', 'number')
a <- as.data.frame(a)
a[, 1] <- c("Manhattan", "Bronx", "Brooklyn", "Queens", "Staten Island")
for (i in 1:5){
...
#### plot of number of house sales in eahc year
```{r}
