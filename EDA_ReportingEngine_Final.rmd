...
model.inputs <- c(borough.name, land.square.feet.name, gross.square.feet.name, sale.year.name, total.units.name, year.built.name, neighborhood.name)
```

```{r data_cleaning}

all.data[, `SALE YEAR` := year(get(sale.date.name))]
all.data[, `SALE MONTH` := month(get(sale.date.name))]
...
Select the inputs you'd like to include in the linear regression model. 
